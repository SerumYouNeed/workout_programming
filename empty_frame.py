...
class EmptyFrame(ctk.CTkFrame):
    ...
    def create_widgets(self, parent):
        ...
        muscle_lbl = ctk.CTkLabel(
            master=self, font=("", 18, "bold"), text="Muscles to load:"
        )
        muscle_lbl.grid(
            column=1,
            row=0,
        ...
        muscle_lbl = ctk.CTkLabel(
            master=self, font=("", 18, "bold"), text="Weekly load:"
        )
        muscle_lbl.grid(column=2, row=0, padx=20, sticky="NSEW")

        # first row
        trainig_program_frame = ctk.CTkFrame(self)
        trainig_program_frame["columns"] = list(
            range(0, parent.number_of_training_days * 2)
        )
        ...
        back_btn = ctk.CTkButton(self, text="Back", font=("", 15), command=back)
...
class MyBtn(ctk.CTkButton):
    def __init__(self, master, column):
        ...
        def btn_add_ex_callback():
            multiplier_muscle_list = self.sql_handler.read_multiplier_muscle(
                master.master.exercise
            )
            ...
            master.master.daily_routines[dict_key].update(
                {master.master.exercise: master.master.sets}
            )

            self.entry_of_this_btn = {master.master.exercise: master.master.sets}

            ex_label = ctk.CTkLabel(master=frame, text=lbl_txt, font=("", 18))
